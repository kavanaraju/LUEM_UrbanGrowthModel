...
fishnet_df <- fishnet %>%
  st_drop_geometry() %>%
  rename(dist_hwy = dist_lr_existing) %>%
  dplyr::select(cell_id, development_change, county, dist_hwy, dist_new, lagDev1)
...
```{r prepare-t2}
# Build a clean pop21_df by spatially joining cell_id from fishnet
# (this preserves ALL fishnet cells and their correct IDs)

pop21_df <- fishnet_pop21 %>%
  # turn the AWI result into centroids so we can join point→polygon
  st_centroid() %>%
  st_join(
    fishnet %>% select(cell_id),   # just the cell_id and geometry of fishnet
    join = st_within
  ) %>%
  # now select only the variables we need
  dplyr::select(cell_id, pop21, inc21, house21) %>%
  st_drop_geometry() %>%
  distinct(cell_id, .keep_all = TRUE)

# sanity check
nrow(pop21_df)        # should equal nrow(fishnet)
sum(is.na(pop21_df))  # hopefully zero or just the imputed inc21 if you did that

# ─────────────────────────────────────────────────────────────────────────────
# Step 15A. Build the 2021 feature table (dat21)
# ─────────────────────────────────────────────────────────────────────────────

# 1. Land‐cover flags for 2021 (from fishnet_lc2)
lc2_df <- fishnet_lc2 %>%
  st_drop_geometry() %>%
  dplyr::select(cell_id, dev2, forest2, farm2, wet2, oth2)  # drop water2 here if you like

# 2. Infrastructure + lag + county (from fishnet)
feat2_df <- fishnet %>%
  st_drop_geometry() %>%
  rename(dist_hwy = dist_lr_existing) %>%    # rename to match model
  dplyr::select(cell_id, dist_hwy, dist_new, lagDev2, county)

# 3. 2021 census estimates (from fishnet_pop21)
pop21_df <- fishnet_pop21 %>%
  st_drop_geometry() %>%
  mutate(cell_id = fishnet$cell_id) %>%      # ensure the same cell_id ordering
  dplyr::select(cell_id, pop21, inc21, house21)

# 4. Left‐join them all and drop any water cells
dat21 <- lc2_df %>%
...
```{r forecast}
# 15.1. Prepare 2021 Feature Table (dat21 already built earlier)
#    (Ensure you've filtered wet2 == 0 as before.)
# 15.2. Predict 2031 Development
...
# 15.3. Join Predictions Back to Spatial Grid
forecast_sf <- left_join(fishnet, dat21, by = "cell_id")

# 15.4. Load Sensitive‐lands Overlays
flood <- st_read("data/flood/fema_flood.shp", quiet = TRUE) %>%
           st_transform(st_crs(forecast_sf))
prot  <- st_read("data/padus/padus_az.shp", quiet = TRUE) %>%
           st_transform(st_crs(forecast_sf))

# 15.5. Flag Risk Overlap
forecast_sf <- forecast_sf %>%
  mutate(
    flood_risk = (pred31 == "1") & lengths(st_intersects(geometry, flood)) > 0,
    prot_risk  = (pred31 == "1") & lengths(st_intersects(geometry, prot))  > 0
  )

# 15.6. Summarize Risk Among Predicted Developments
risk_sum <- forecast_sf %>%
  filter(pred31 == "1") %>%
  st_drop_geometry() %>%
  summarize(
    total_cells = n(),
    pct_flood   = mean(flood_risk) * 100,
    pct_prot    = mean(prot_risk)  * 100
  )
knitr::kable(risk_sum, digits = 1,
             caption = "2031 Forecast: Percent of Predicted Cells at Risk")

# 15.7. Map the 2031 Forecast
library(ggplot2)
ggplot(forecast_sf) +
  ...
  labs(
    title = "Projected 2031 Development in Phoenix MSA",
    subtitle = glue::glue("Threshold = {threshold * 100}%"),
    caption  = "Light‐rail extension scenario"
  ) +
  theme_void()
```
