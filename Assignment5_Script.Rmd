...
```{r spatial-lag}
# Coordinates for KNN: fishnet centroids
grid_xy <- st_coordinates(st_centroid(fishnet))

# Ensure dev1 and dev2 are numeric vectors (0/1)
fishnet_lc1 <- fishnet_lc1 %>% mutate(dev1 = as.numeric(dev1))
fishnet_lc2 <- fishnet_lc2 %>% mutate(dev2 = as.numeric(dev2))

# Extract points for cells that developed in t1 and t2
points1 <- fishnet_lc1 %>%
  filter(.data$dev1 == 1) %>%
  st_centroid() %>%
  st_coordinates()
...
# Compute spatial lag distances (average to 2 nearest developed cells)
fishnet <- fishnet %>% mutate(
  lagDev1 = nn_function(grid_xy, points1, k = 2),  # t1 spatial lag
  lagDev2 = nn_function(grid_xy, points2, k = 2)   # t2 spatial lag
)
```
