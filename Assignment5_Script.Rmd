...
# pull in just the grid and its predictors
pred_sf <- fishnet %>% 
  st_as_sf() %>% 
  dplyr::select(
    dist_lr_existing,   # existing-rail distance
    dist_new,           # proposed-extension distance
    lagDev1,            # spatial lag
    pop11,              # 2011 population
    inc11,              # 2011 median income
    house11             # 2011 housing units
  )

# pivot long for faceting
pred_long <- pred_sf %>%
...
# reattach geometry
pred_long_sf <- left_join(
  pred_long,
  fishnet %>% dplyr::select(cell_id, geometry),
  by = "cell_id"
) %>% st_as_sf()
