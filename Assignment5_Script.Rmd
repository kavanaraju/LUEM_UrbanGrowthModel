...
aggregateRaster <- function(inputRasterList, fishnet) {
  # 1. Start with your fishnet grid
  result <- fishnet
  
  # 2. For each binary raster layer in the list...
  for(r in inputRasterList) {
    nm <- names(r)  # e.g. "dev1", "forest1", etc.
    
    # 3. Count how many cells == 1 fall into each polygon
    counts <- raster::extract(
      r, 
      result, 
      fun = function(vals, ...) sum(vals == 1, na.rm = TRUE)
    )
    
    # 4. Create a 0/1 flag and add as a new column
    result[[nm]] <- ifelse(counts > 0, 1, 0)
  }
  
  # 5. Return the enriched fishnet
  return(result)
}
```
