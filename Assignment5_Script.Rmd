...
ggplot(forecast_sf) +
  geom_sf(aes(fill = pred31), color = NA) +
  geom_sf(data = prot, colour = "darkgreen", fill = NA, size = 0.3) +
  geom_sf(data = phx_msa, fill = NA, colour = "black") +
  scale_fill_manual(
    values = c("0" = "grey80", "1" = "darkgreen"),
    labels = c("No Dev","Developed"),
    name   = "2031 Dev"
  ...
  labs(
    title    = "Projected 2031 Development & Protected Areas",
    subtitle = glue::glue("Threshold = {threshold * 100}%")
  ) +
  theme_void()
```
