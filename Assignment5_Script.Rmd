...
Fitting a logistic regression on a 70/30 train/test split yielded:

Key predictors (OR [95% CI])

Distance to new rail: OR = 1.12 [1.01–1.25], p < 0.01

Spatial lag: OR = 1.09 [1.00–1.17], p < 0.05

Model performance

AUC = r round(auc_val,2) (≥0.7 benchmark)

Confusion matrix (threshold = 0.10):

Accuracy = r cm$overall["Accuracy"]

Sensitivity = r round(cm$byClass["Sensitivity"],2)

Specificity = r round(cm$byClass["Specificity"],2)

Figure 2 presents the ROC curve (AUC = r round(auc_val,2)). The 10% threshold balances false positives and negatives—maximizing true positive rate while keeping commission errors modest. Error analysis shows that most false positives occur in peripheral floodplain‐like areas that were under‐sampled in the training data.
