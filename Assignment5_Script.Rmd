...
# 1. Pull out the 2021 pop/inc/house and attach cell_id
pop21_df <- fishnet_pop21 %>%
  st_drop_geometry() %>%
  mutate(cell_id = fishnet$cell_id) %>%
  select(cell_id, pop21, inc21, house21)

# 2. Join 2011 & 2021 socioeconomic and the infra/lag columns into fishnet
fishnet_map <- fishnet %>%
  left_join(pop11_df, by = "cell_id") %>%       # adds pop11, inc11, house11
  left_join(pop21_df, by = "cell_id")           # adds pop21, inc21, house21

# 3. Now it has dist_lr_existing, dist_new, lagDev1, pop11, inc11, house11, etc.
#    Melt to long for mapping
library(tidyr)
fishnet_long <- fishnet_map %>%
  select(cell_id, dist_lr_existing, dist_new, lagDev1, pop11, inc11, house11) %>%
  pivot_longer(-cell_id, names_to = "variable", values_to = "value") %>%
  left_join(fishnet_map %>% select(cell_id, geometry), by = "cell_id") %>%
  st_as_sf()

# 4. Plot a small faceted map of all six predictors
ggplot(fishnet_long) +
  ...
  labs(fill = NULL, title = "Predictor surfaces (2011 & 2021 variables)")
```
...
- False positives concentrate on fringe cells (e.g., wash corridors) under-represented in training, leading to over-prediction.
